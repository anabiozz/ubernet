...
func parseConfig() *Config {
	...
	var timeoutMS int
	// Flag definition
	flag.IntVar(&timeoutMS, "t", 10000, "Timeout in millisecond for the whole checking process(domain resolving is included)")
	flag.StringVar(&conf.Addr, "a", "127.0.0.1:9595", "TCP address to test")
	flag.IntVar(&conf.Requests, "n", 10, "Number of requests to perform")
	flag.IntVar(&conf.Concurrency, "c", 10, "Number of checks to perform simultaneously")
...
func (cc *ConcurrentChecker) doCheck() {
	...
	default:
		...
		if _, ok := err.(*ubernet.ErrConnect); ok {
